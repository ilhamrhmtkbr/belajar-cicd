...
class InstructorSection extends Model
{
    ...
    public function scopeGetByInstructor(Builder $query, string $username): Builder
    {
        return $query->whereHas('instructorCourse', function ($query) use ($username) {
            $query->where('instructor_id', $username);
        })->select(['id', 'instructor_course_id', 'title', 'order_in_course', 'created_at', 'updated_at']);
    }
}
