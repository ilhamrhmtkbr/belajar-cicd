                ...
                {courseDetail &&
                    <div className={'max-width-1000 card-wrapper'}>
                        ...
                        <img className={'max-width-500 border-style-default radius-m object-fit-cover'}
                             src={import.meta.env.VITE_APP_IMAGE_COURSE_URL + courseDetail?.course?.image} alt={courseDetail?.course?.title}
                             style={{
                                 minHeight: 275,
                                 maxHeight: 275
                             }}
                        ...
                        {loadingRequest ?
                            ...
                            <div onClick={() => handleLike(courseDetail?.course?.id)} style={{width: 17}}
                                 className={'flex-aic-jcc cursor-pointer'}>
                                <SvgComp rule={`svg-m svg-fill-blue ${courseDetail?.isLikes ? 'fill-blue' : ''}`} file={'sprite'}
                                         icon={'like'}/>
                        ...
                        <div className={'flex-aic-jcs gap-l'}>
                            <div className={'max-width-300 card-wrapper replace-shadow-with-border'}>
                                <h3>{t('price')}</h3>
                                <div className="badge badge-small bg-primary" style={{ color: 'white'}}>{formatNumber(courseDetail?.course?.price)}</div>
                            </div>
                            <div className={'max-width-300 card-wrapper replace-shadow-with-border'}>
                                <h3>{t('level')}</h3>
                                <div className="badge capitalize badge-small bg-primary" style={{ color: 'white'}}>{courseDetail?.course?.level}</div>
                            </div>
                            <div className={'max-width-300 card-wrapper replace-shadow-with-border'}>
                                <h3>{t('status')}</h3>
                                <div className={`badge capitalize badge-small bg-${courseDetail?.course?.status === 'free' ? 'success' : 'primary'}`} style={{ color: 'white'}}>{courseDetail?.course?.status}</div>
                            </div>
                        </div>
